...
it('enforces runtime ownership, unique attempts/iterations, confirmation checks and restricted history deletion', () => {
  ...
  expect(() => db.prepare("INSERT INTO workspace_checkpoints(id,run_id,stage_attempt_id,checkpoint_kind,repository_head,baseline_fingerprint,resulting_fingerprint,staged_patch_object_id,unstaged_patch_object_id,untracked_manifest_object_id,submodule_manifest_object_id,created_at) VALUES('cp','run1','a2','progress','h','b','r','o','o','o','o',?)").run(now)).toThrow(/CHECKPOINT_OWNERSHIP/);
  expect(() => db.prepare("INSERT INTO confirmation_requests(id,run_id,stage_run_id,confirmation_type,request_summary,action_hash,nonce_hash,safety_baseline_object_id,requested_installation_id,status,requested_at,expires_at) VALUES('c','run1','s2','x','x','a','n','o','i','pending',?,?)").run(now, now)).toThrow(/CONFIRMATION_OWNERSHIP/);
  expect(() => db.prepare("INSERT INTO confirmation_requests(id,run_id,stage_run_id,confirmation_type,request_summary,action_hash,nonce_hash,safety_baseline_object_id,requested_installation_id,status,requested_at,expires_at) VALUES('bad','run1','s1','x','x','a','n','o','i','made_up',?,?)").run(now, now)).toThrow(/CHECK/);
  db.prepare("INSERT INTO confirmation_requests(id,run_id,stage_run_id,confirmation_type,request_summary,action_hash,nonce_hash,safety_baseline_object_id,requested_installation_id,status,requested_at,expires_at) VALUES('good','run1','s1','x','x','a','n','o','i','pending',?,?)").run(now, now);

...
function scenario(policy: 'pause' | 'fail') {
  ...
  fixture.db.prepare(`INSERT INTO run_snapshots
    ...
    VALUES('run',?,?,?,?,?,'head',?,?,?,?,?,?)`)
    .run(workflow.id, fixture.object.id, fixture.object.id, fixture.object.id, fixture.object.id,
  ...
  const request = service.request({ runId: 'run', stageRunId: 'confirm', stageAttemptId: 'confirm-attempt',
    type: 'release', summary: 'release', actionHash: 'action', safetyBaselineObjectId: fixture.object.id, installationId: 'install' });
  ...
  service.submitDecision({ confirmationRequestId: request.id, nonce: request.nonce, exactActionHash: 'action', decision: 'reject',

...
it('uses fixed executable/args, rejects arbitrary keys, redacts output and supports fixed reconciliation', async () => {
  const registry = DriverRegistry.forTestFixtures([{ actionType: 'fixture', executable: '/bin/echo', allowedParameterKeys: ['version'], fixedArgs: ['password=secret'], reconcileArgs: ['reconcile'], timeoutMs: 1000 }]);

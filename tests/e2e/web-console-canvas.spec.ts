...
test("n8n style authoring opens a role market without exposing Run controls", async ({ page }) => {
  ...
  await page.getByRole("searchbox", { name: "搜索角色" }).fill("architecture");
  await page.getByRole("button", { name: /Architecture/ }).click();
  await expect(page.getByRole("dialog", { name: "添加节点" })).toHaveCount(0);
  for (const name of ["开始", "暂停", "恢复", "取消", "重试", "部署"])

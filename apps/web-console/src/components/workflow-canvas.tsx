...
export function WorkflowCanvas({ envelope, roles, label, readonly = false, stageStates: stageStatesInput, onChange, onSelect }: Props) {
  ...
  const stageLabel = useCallback((stage: WorkflowStage) => {
    const direct = label(stage.key);
    if (direct !== stage.key) return direct;
    const role = roles.find((item) => item.current_version_id === stage.role_version_id);
    if (role) {
      const localizedRole = label(role.slug);
      return localizedRole === role.slug ? role.name : localizedRole;
    }
    return label(stage.key.replace(/-\d+$/, ""));
  }, [label, roles]);
  const authoredNodes = useMemo<Node[]>(() => {
    const stages = envelope.data.stages.filter((stage) => !groupFor(stage.key)).map((stage) => ({
      id: stage.key, type: "stage", position: positions[stage.key]!, data: { stage, label: stageLabel(stage), selected: selected === stage.key, state: stageStates[stage.key] ?? "queued", stateLabel: t(stageStates[stage.key] ?? "queued"), meta: stage.mandatory_gate ? t("mandatoryGate") : stage.optional ? t("optionalStage") : t("stage"), quickAddLabel: t("addNode"), ...(!readonly ? { onQuickAdd: (key: string) => { setSelected(key); setPaletteOpen(true); } } : {}) },
    }));
  ...
  }, [collapsed, envelope, groupFor, onChange, positions, readonly, selected, stageLabel, stageStates, t]);
  const [nodes, setNodes, applyNodeChanges] = useNodesState<Node>([]);
  const hasAuthoredNodes = authoredNodes.length > 0;
  ...
  const edges = useMemo<Edge[]>(() => {
    ...
    return envelope.data.edges.flatMap((edge, index) => {
      ...
      const className = [stageStates[edge.from] === "failed" ? "edge-failed" : "", selectedEdge === id ? "edge-selected" : ""].filter(Boolean).join(" ");
      return [{ id, source, target, type: "smoothstep", animated: !reducedMotion && Boolean(stageStates[edge.from] === "running"), ...(edge.edge_type === "on_success" ? { label: t("onSuccess") } : {}), ...(className ? { className } : {}) }];
    });
  ...
  // xyflow 对程序化 fitView 也会触发 onMoveEnd，此时 event 为 null。
  // 不过滤的话一打开编辑器就显示"尚未保存的修改"，而且每次平移缩放都吃掉一格撤销栈。
  const onMoveEnd: OnMoveEnd = (event, viewport) => { if (!readonly && onChange && event) onChange(updateCanvasViewport(envelope, viewport)); };
  ...
  return <div className={`workflow-canvas-layout ${readonly ? "is-readonly" : ""}`}>
    <section ref={stageRef} className="canvas-stage n8n-canvas" data-canvas-ready={canvasReady} aria-label={readonly ? t("runCanvas") : t("workflowEditor")} tabIndex={0} onKeyDown={(event) => {
      ...
      if (!readonly && (event.key === "Delete" || event.key === "Backspace")) {
        ...
        if (next !== envelope) { event.preventDefault(); onChange?.(next); select(undefined); setSelectedEdge(undefined); }
      }
    }}>
      ...
      {!hasAuthoredNodes && <div className={`canvas-empty-state ${readonly ? "is-readonly" : ""}`} data-testid="canvas-empty-state"><span className="canvas-empty-mark" aria-hidden>{readonly ? "—" : "＋"}</span><h2>{t(readonly ? "emptyRunCanvasTitle" : "emptyCanvasTitle")}</h2><p>{t(readonly ? "emptyRunCanvasDescription" : "emptyCanvasDescription")}</p>{!readonly && <button className="button primary" type="button" onClick={() => setPaletteOpen(true)}>{t("addNode")}</button>}</div>}
      {diagnostics.length > 0 && hasAuthoredNodes && !readonly && <div className="canvas-diagnostics" role="status">{diagnostics.map((item) => <span key={`${item.code}-${item.stageKeys.join()}`}>● {t(`graph_${item.code}`)}：{item.stageKeys.map(label).join("、")}</span>)}</div>}
    </section>
      {paletteOpen && !readonly && <CanvasDrawer title={t("addNode")} onClose={() => setPaletteOpen(false)} className="role-market"><label className="field"><span>{t("searchRoles")}</span><input autoFocus type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("searchRoles")} /></label>{groups.map((name) => <section className="role-market-group" key={name}><h3>{t(name)}</h3>{filteredRoles.filter((role) => category(role.slug) === name).map((role) => <button className="palette-role" type="button" draggable key={role.id} onDragStart={(event) => event.dataTransfer.setData("application/project-orchestrator-role", role.id)} onClick={() => addRole(role)}><span><strong>{label(role.slug)}</strong><small>{role.slug}</small></span><b aria-hidden>＋</b></button>)}</section>)}{!filteredRoles.length && <p className="muted">{t("noData")}</p>}</CanvasDrawer>}
  </div>;
}

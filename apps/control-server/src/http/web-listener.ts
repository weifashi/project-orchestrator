...
// 登录页的 favicon。与 apps/web-console/index.html 里的那份逐字一致，
// tests/integration/web-static-security 会交叉校验，改一边必须同步另一边。
const FAVICON_HREF = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><rect width='64' height='64' rx='16' fill='%23ff6d5a'/><text x='32' y='42' text-anchor='middle' fill='%2325110e' font-family='system-ui,-apple-system,Segoe UI,sans-serif' font-size='30' font-weight='800'>PO</text></svg>";
const faviconLink = `<link rel="icon" type="image/svg+xml" href="${FAVICON_HREF}">`;
...
const bootstrapPage = (styleNonce: string, registering: boolean, locale: BootstrapLocale, error?: string) => {
  const copy = bootstrapCopy[locale];
  return `<!doctype html>
<html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${registering ? copy.registerTitle : copy.loginTitle} · Project Orchestrator</title>${faviconLink}

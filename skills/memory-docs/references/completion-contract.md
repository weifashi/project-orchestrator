# Completion contract

A successful output is accepted only when:

- `memory_records` is present and references durable artifact content.
- `delivery_summary` is present and references durable artifact content.
- `provenance_map` is present and references durable artifact content.
- Risks and next-stage notes are explicit, even when empty.
- Artifact and evidence ids resolve to immutable objects.
- The role makes none of the forbidden claims in `SKILL.md`.
